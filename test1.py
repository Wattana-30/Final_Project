...
def retinex(image, sigma=100):
    return cv2.xphoto.createTonemapDurand(gamma=2.2).process(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))

def save_processed_image(image, filename, save_folder):
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    save_path = os.path.join(save_folder, filename)
    cv2.imwrite(save_path, image)
    print(f"บันทึกภาพที่ปรับปรุงแล้ว: {save_path}")
...
def process_images_from_folder(folder_path, save_folder):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.jpg') or filename.endswith('.png'):
            ...
            save_processed_image(bright_image, filename, save_folder)
            ...
            # การแปลงภาพเป็น Grayscale สำหรับ GLCM
            gray_image = cv2.cvtColor(bright_image, cv2.COLOR_BGR2GRAY)
            glcm_features = extract_glcm_features(gray_image)

            # การใช้ Retinex
            retinex_image = retinex(bright_image)
            retinex_mean = np.mean(retinex_image)
            retinex_std = np.std(retinex_image)
            retinex_features = {'retinex_mean': retinex_mean, 'retinex_std': retinex_std}

            # รวมคุณลักษณะทั้งหมด
            all_features = {**features, **color_ratios, **glcm_features, **retinex_features, 'filename': filename}
...
save_folder = r'E:\Project\Program'
process_images_from_folder(folder_path, save_folder)
